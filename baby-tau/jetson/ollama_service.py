...
class OllamaService:
    def __init__(self, useNgrok=True, ollama_url="http://ollama:11434"):
        load_dotenv()
        self.useNgrok = useNgrok
        self.api_url = ollama_url  # Use provided Ollama URL
        self.headers = {
    ...
    def transcribe_audio(self, audio_data, rate=16000):
        ...
        api_endpoint = f"{self.api_url}/api/audio/transcriptions"
        files = {
            'file': ('audio.wav', audio_data, 'audio/wav'),
            'model': (None, "whisper-1")  # Or any other model name
        }
        try:
            response = requests.post(api_endpoint, files=files, headers={})
